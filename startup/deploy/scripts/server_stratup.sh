...
if [ -f ./ancient-empire-server/ancient-empire-server.jar ]; then
    ...
    nohup java -jar -Dfile.encoding=UTF-8 -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5005 ./ancient-empire-server/ancient-empire-server.jar jfile=config/application.properties >/dev/null &2>~/runtime/log/ae.server.start.log
    strat_time=$(date "+%Y-%m-%d %H:%M:%S")
